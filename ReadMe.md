...
This is a sample code for Melissa's Global Email API.
...
Inside the folder will be the build under `globalemail` as well as an all-in-one `globalemail.zip`.
...
To run the example, navigate to `./target/globalemail/bin/` and run `globalemail.bat`.
